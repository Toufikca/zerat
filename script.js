javascript_20260210_french_complete.js - Système complet en français
...
class EnhancedFootballManagerApp {
    ...
    loadConfig() {
        const defaultConfig = {
            APP_NAME: 'Smart Coach's Bag par ZERAZEHI.T',
            VERSION: '2.1.0',
        ...
        return `
            <div class="header-left">
                ...
                <a href="#" class="logo" onclick="app.navigateTo('dashboard')">
                    ...
                    <div class="logo-text">
                        <span class="primary">Smart Coach's Bag par ZERAZEHI.T</span>
    ...
    getSidebarTemplate() {
        ...
        return `
            ...
            <div class="sidebar-footer">
                ...
                <div class="copyright">© 2027 ZERAZEHI.T</div>
            </div>
        `;
    }
    ...
    async handleFormSubmit(form) {
        ...
        try {
            ...
            if (formId === 'add-player-form') {
            ...
            } else if (formId === 'edit-training-form') {
                const trainingId = form.dataset.trainingId;
                result = await this.api.request(`/trainings/${trainingId}`, {
                    method: 'PUT',
                ...
                if (index !== -1) {
                    trainings[index] = { ...trainings[index], ...result };
                    this.store.set('trainings', trainings);
                }
                this.notificationManager.show('Session modifiée avec succès', 'success');
            } else if (formId === 'edit-match-form') {
                const matchId = form.dataset.matchId;
                result = await this.api.request(`/matches/${matchId}`, {
                    method: 'PUT',
                    body: JSON.stringify(data)
                });
                ...
                if (index !== -1) {
                    ...
                    this.store.set('matches', matches);
                }
                this.notificationManager.show('Match modifié avec succès', 'success');
            }
            ...
            <form id="edit-player-form" data-player-id="${player.id}" class="form-container">
                ...
                <div class="form-group">
                            ...
                            <div class="login-container">
                                <div class="login-card">
                                    <div class="login-header">
                                        <i class="fas fa-futbol"></i>
                                        <h1>Smart Coach's Bag par ZERAZEHI.T</h1>
                    ...
                    editTraining(trainingId) {
                        ...
                        const modal = this.createModal({
                            title: 'Modifier la séance d\'entraînement',
                            body: this.getEditTrainingForm(training),
                            footer: `
                                <button class="btn btn-secondary" onclick="app.closeModal()">Annuler</button>
                                <button type="submit" form="edit-training-form" class="btn btn-primary">Sauvegarder</button>
                            `,
                            size: 'lg'
                        });
                        
                        document.body.appendChild(modal);
                        this.bindFormValidation('edit-training-form');
                    }

                    getEditTrainingForm(training) {
                        return `
                            <form id="edit-training-form" data-training-id="${training.id}" class="form-container">
                                <div class="form-row">
                                    <div class="form-group">
                                        <label class="label-required">Date</label>
                                    ...
                                    <div class="form-group">
                                        <label class="label-required">Type</label>
                                        <select name="type" required>
                                            <option value="Technique" ${training.type === 'Technique' ? 'selected' : ''}>Technique</option>
                                            <option value="Physique" ${training.type === 'Physique' ? 'selected' : ''}>Physique</option>
                                            <option value="Tactique" ${training.type === 'Tactique' ? 'selected' : ''}>Tactique</option>
                                            <option value="Récupération" ${training.type === 'Récupération' ? 'selected' : ''}>Récupération</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="form-group">
                                        <label class="label-required">Durée</label>
                                        <input type="text" name="duration" value="${training.duration}" required>
                                    </div>
                                    <div class="form-group">
                                        <label class="label-required">Intensité</label>
                                        <select name="intensity" required>
                                            <option value="Basse" ${training.intensity === 'Basse' ? 'selected' : ''}>Basse</option>
                                            <option value="Moyenne" ${training.intensity === 'Moyenne' ? 'selected' : ''}>Moyenne</option>
                                            <option value="Haute" ${training.intensity === 'Haute' ? 'selected' : ''}>Haute</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="label-required">Participants</label>
                                    <input type="number" name="participants" value="${training.participants}" min="1" max="30" required>
                                </div>
                                <div class="form-group">
                                    <label class="label-required">Entraîneur</label>
                                    <input type="text" name="coach" value="${training.coach}" required>
                                </div>
                            </form>
                        `;
                    }
                    ...
                    deleteTraining(trainingId) {
                        this.showConfirmationModal({
                            ...
                            onConfirm: async () => {
                                try {
                                    await this.api.request(`/trainings/${trainingId}`, { method: 'DELETE' });
                                    this.store.remove('trainings', trainingId);
                                    this.notificationManager.show('Séance supprimée avec succès', 'success');
                                    if (this.currentPage === 'training') {
                                        await this.navigateTo('training');
                                    }
                                } catch (error) {
                                    this.notificationManager.show('Échec de la suppression', 'error');
                                }
                            }
                        });
                    }
